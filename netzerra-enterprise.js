...
function executePurchase() {
  ...
  toast(`🚀 Executing Trade on Exchange & Syncing payload to KNCR API Gateway...`, 'info');
...
<div class="page">
  ...
  <div class="cert-footer">
    ...
    This certificate is generated by Netzerra MVP software. It demonstrates KNCR API integrations.
    Official KNCR credit retirement must be completed on kncr.go.ke.
  </div>
</div>
</body>
</html>`;
...
function respondToRFQ(rfqId) {
  ...
  B2B.contracts.unshift({
    ...
    price: rfq.maxPrice,
    total: rfq.quantity * rfq.maxPrice,
...
function showCertificateOfRetirement(certId, amount, buyer, seller, ref) {
  const modalHTML = `
    <div id="cert-modal" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.8);z-index:9999;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(10px)">
      <div style="background:#0a1f14;border:2px solid var(--forest);width:90%;max-width:700px;border-radius:12px;padding:40px;color:#fff;position:relative;box-shadow:0 20px 50px rgba(0,0,0,.5);text-align:center;">
        <button onclick="document.getElementById('cert-modal').remove()" style="position:absolute;top:20px;right:20px;background:none;border:none;color:#fff;font-size:24px;cursor:pointer">✕</button>
        <div style="font-size:40px;margin-bottom:10px">🏅</div>
        <h2 style="color:var(--gold);font-family:'Times New Roman', serif;font-size:28px;text-transform:uppercase;letter-spacing:2px;margin-bottom:5px">Certificate of Retirement</h2>
        <div style="font-size:12px;color:rgba(255,255,255,.5);margin-bottom:30px">KENYA NATIONAL CARBON REGISTRY (KNCR)</div>
        
        <p style="font-size:16px;color:#fff;line-height:1.6;max-width:80%;margin:0 auto 30px;">
          This is to certify that <strong style="color:var(--mint)">${amount.toLocaleString()} tCO₂e</strong> 
          have been successfully transferred and retired on behalf of:<br>
          <strong style="font-size:22px;display:block;margin:15px 0;color:var(--leaf);text-transform:uppercase">${buyer}</strong>
        </p>
        
        <table style="width:100%;text-align:left;background:rgba(255,255,255,.05);border-collapse:collapse;margin-bottom:30px">
          <tr style="border-bottom:1px solid rgba(255,255,255,.1)">
            <td style="padding:10px;font-size:12px;color:rgba(255,255,255,.5)">Serial Number</td>
            <td style="padding:10px;font-family:monospace;color:#fff">${certId}</td>
          </tr>
          <tr style="border-bottom:1px solid rgba(255,255,255,.1)">
            <td style="padding:10px;font-size:12px;color:rgba(255,255,255,.5)">Sourced From</td>
            <td style="padding:10px;font-family:monospace;color:#fff">${seller}</td>
          </tr>
          <tr>
            <td style="padding:10px;font-size:12px;color:rgba(255,255,255,.5)">Ledger Sync</td>
            <td style="padding:10px;font-family:monospace;color:var(--teal)">✅ KNCR Article 6 Corresponding Adjustment Applied</td>
          </tr>
        </table>
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;border-top:1px dashed rgba(255,255,255,.2);padding-top:20px;">
          ...
          <div style="text-align:right">
            <div style="font-size:11px;color:rgba(255,255,255,.5);margin-bottom:8px">Signed electronically via S.A. System</div>
            <button class="gcis-btn" style="background:var(--leaf);color:#0a1f14;font-weight:600;border:none">Download PDF</button>
          </div>
        </div>
      </div>
    </div>
...
function renderNEMAOversight() {
  ...
  let userProjects = (window.S?.kncr?.projects || []).map(p => ({
    id: p.id,
    name: p.name,
    developer: window.S?.user?.org || 'Netzerra Consultant',
    county: p.county,
    sector: p.sector,
    credits: p.credits,
    status: ['Draft', 'PDD Submitted', 'Validation', 'DNA Review', 'Registered', 'Credits Live'][p.step - 1] || 'Draft',
    step: p.step,
    cda: p.sector === 'borehole' || p.sector === 'livestock' ? 40 : 25,
    cdaCompliant: true,
    lastAudit: 'Pending'
  }));

  let projects = [...userProjects, ...NEMA_DATA.projects];
  if (isCounty && county) {
